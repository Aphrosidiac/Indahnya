...
export async function useGuestEvent() {
  ...
  useSeoMeta({
    ...
    ogType: 'website', ogSiteName: 'Indahnya',
  });
  await req;
  if (error.value) throw createError({ statusCode: error.value.statusCode ?? 404, statusMessage: 'Majlis tak jumpa', fatal: true });
  const ev = computed(() => data.value!.event);
  const me = computed(() => data.value?.me ?? null);
  const t = useT(() => ev.value.settings.locale);
  const displayName = computed(() => ev.value.names.b ? `${ev.value.names.a} & ${ev.value.names.b}` : ev.value.title);
  return {
    slug, ev, me, ready: computed(() => data.value?.ready ?? 0), t, displayName, refresh,
