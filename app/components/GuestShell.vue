...
<template>
  <div class="flex min-h-screen flex-col bg-surface-50">
    <header class="sticky top-0 z-20 bg-surface-50/85 backdrop-blur-md">
      <div class="mx-auto flex h-14 w-full max-w-[1100px] items-center justify-between gap-3 px-4">
        <NuxtLink :to="base" class="min-w-0">
          ...
          <span v-if="ev.date" class="block truncate text-[12px] leading-4 text-ink-500">{{ fmtDate(ev.date) }}</span>
        </NuxtLink>
        <nav v-if="tabs.length > 1" class="hidden items-center gap-0.5 rounded-[11px] bg-sand p-[3px] sm:inline-flex" aria-label="Bahagian">
    ...
    <main class="mx-auto w-full max-w-[1100px] flex-1 px-4 pb-28 pt-2 sm:pb-12">
      <slot />
    </main>

    <footer v-if="ev.badge" class="mx-auto w-full max-w-[1100px] px-4 pb-36 text-center text-[12px] leading-4 text-ink-400 sm:pb-8">
    ...
    <nav v-if="tabs.length > 1" class="fixed inset-x-0 bottom-0 z-20 border-t border-line-100 bg-surface-0/95 pb-[env(safe-area-inset-bottom)] backdrop-blur-md sm:hidden" aria-label="Bahagian">
      <div class="mx-auto grid h-16 max-w-[520px]" :style="{ gridTemplateColumns: `repeat(${tabs.length}, minmax(0, 1fr))` }">
        <NuxtLink v-for="tb in tabs" :key="tb.key" :to="tb.to" class="flex flex-col items-center justify-center gap-1 text-[11px] font-medium leading-4 transition-colors"
