...
export default defineEventHandler(async (event) => {
  ...
  if (q.format === 'png') {
    ...
    return QRCode.toBuffer(url, { type: 'png', width: Math.min(Math.max(Number(q.size) || 1024, 128), 4096), margin: 1, errorCorrectionLevel: 'M', color: { dark: '#1a1a1a', light: '#ffffff' } });
  }
