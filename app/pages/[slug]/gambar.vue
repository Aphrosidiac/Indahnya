...
import { Camera, Video, X, Download, Trash2, ChevronLeft, ChevronRight, RefreshCw, Check, AlertCircle, Heart, PartyPopper, Smile } from 'lucide-vue-next';
import { Btn, Tabs, EmptyState, Sk, Modal, Field, Alert, Meter, useUi } from '~/ui';

definePageMeta({ layout: 'bare' });
const { slug, ev, me, t, displayName, setMe, setMeta } = await useGuestEvent();
const ui = useUi();
...
watch(() => up.active.value, async (a, was) => {
  if (was && !a) {
    await load();
    if (up.done.value) ui.ok(t('gallery.done', { n: up.done.value }));
...
<template>
  <GuestShell :ev="ev" :title="displayName" :t="t">
    <div class="reveal pt-3">
      ...
      <Alert v-if="!ev.uploadsOpen" tone="muted" :title="t('gallery.closed')" class="mt-3">{{ t('gallery.closed.sub') }}</Alert>
      ...
      <div v-else class="reveal-flat mt-4 grid grid-cols-3 gap-1 sm:grid-cols-4 sm:gap-2 lg:grid-cols-6">
        <button v-for="m in items" :key="m.id" type="button" class="group relative aspect-square overflow-hidden rounded-[8px] bg-sand sm:rounded-[10px]" @click="viewing = m">
    ...
    <div v-if="ev.uploadsOpen" class="fixed inset-x-0 bottom-[calc(64px+env(safe-area-inset-bottom))] z-20 flex justify-center px-4 pb-3 sm:hidden">
  ...
  <Teleport to="body">
    ...
    <Transition name="pop">
      <div v-if="sheet" class="card fixed inset-x-0 bottom-0 z-50 mx-auto max-h-[80vh] w-full max-w-[560px] overflow-hidden rounded-b-none sm:bottom-6 sm:rounded-b-[16px]" role="dialog" aria-modal="true">
        <div class="flex items-center justify-between gap-3 px-5 pb-3 pt-4">
          <div><p class="text-[15px] font-semibold leading-6 text-ink-900">{{ up.active.value ? t('gallery.uploading') : (up.failed.value ? t('gallery.failed', { n: up.failed.value }) : t('gallery.done', { n: up.done.value })) }}</p><p class="text-[12px] leading-4 text-ink-500">{{ up.done.value }} / {{ up.items.value.length }}</p></div>
          <button type="button" class="grid size-8 place-items-center rounded-[8px] text-ink-500 hover:bg-sand hover:text-ink-900" aria-label="Tutup" @click="closeSheet"><X class="size-[18px]" :stroke-width="1.75" /></button>
        </div>
          ...
          <li v-for="it in up.items.value" :key="it.key" class="flex items-center gap-3 px-5 py-2.5">
            ...
            <span class="min-w-0 flex-1">
              ...
              <span class="block truncate text-[12px] leading-4" :class="it.state === 'failed' ? 'text-danger-600' : 'text-ink-500'">
                {{ it.state === 'failed' ? it.error : it.state === 'ready' ? '✓' : it.state === 'processing' ? t('gallery.processing') : it.state === 'uploading' ? `${it.pct}%` : '…' }}
              </span>
            </span>
            <Check v-if="it.state === 'ready'" class="size-5 shrink-0 text-success-600" :stroke-width="2" aria-hidden="true" />
            <button v-else-if="it.state === 'failed'" type="button" class="grid size-8 shrink-0 place-items-center rounded-[8px] text-ink-600 hover:bg-sand" aria-label="Retry" @click="up.retry(it)"><RefreshCw class="size-4" :stroke-width="1.75" /></button>
            <AlertCircle v-else-if="false" class="size-5 text-danger-600" />
        ...
        <div class="flex gap-2 border-t border-line-100 bg-surface-50 px-5 py-3">
          <Btn variant="secondary" block @click="pick"><Camera class="size-4" :stroke-width="1.75" aria-hidden="true" />+</Btn>
          <Btn variant="primary" block :disabled="up.active.value" @click="closeSheet">OK</Btn>
        </div>
      </div>
    </Transition>
  </Teleport>
  ...
  <Teleport to="body">
    <Transition name="veil">
      <div v-if="viewing" class="fixed inset-0 z-[60] flex flex-col bg-ink-900/95" @touchstart.passive="onTouchStart" @touchend.passive="onTouchEnd" @click.self="viewing = null">
        <div class="flex items-center justify-between gap-3 p-3 text-white">
          <div class="min-w-0 px-1">
            ...
            <p class="truncate text-[12px] leading-4 text-white/60">{{ fmtDateTime(viewing.takenAt ?? viewing.createdAt) }}</p>
          </div>
          <div class="flex shrink-0 items-center gap-1">
            <a :href="viewing.url" download class="grid size-9 place-items-center rounded-full text-white/80 transition-colors hover:bg-white/10 hover:text-white" :aria-label="t('gallery.download')"><Download class="size-[18px]" :stroke-width="1.75" /></a>
        ...
        <button v-if="idx > 0" type="button" class="absolute left-2 top-1/2 z-10 hidden size-10 -translate-y-1/2 place-items-center rounded-full bg-white/10 text-white hover:bg-white/20 sm:grid" aria-label="←" @click="step(-1)"><ChevronLeft class="size-6" :stroke-width="1.75" /></button>
        <button v-if="idx < items.length - 1" type="button" class="absolute right-2 top-1/2 z-10 hidden size-10 -translate-y-1/2 place-items-center rounded-full bg-white/10 text-white hover:bg-white/20 sm:grid" aria-label="→" @click="step(1)"><ChevronRight class="size-6" :stroke-width="1.75" /></button>
